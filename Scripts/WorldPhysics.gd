...
func get_terrain_radius():
	return _terrain_radius


func get_total_accel(position:Vector3, velocity:Vector3)->Vector3:
	var _depth:float = get_depth(position);
	
	if is_in_outer_space(position):
		return go_home_accel(position);
	
	var _half_ter_width:float = sqrt(SQ_HALF_TER_WIDTH)
	var _drag_coef:float = MAX_DRAG_COEF if get_depth(position) > 0.0 else MAX_DRAG_COEF * pow((_half_ter_width-_get_unsigned_local_height(position)) / _half_ter_width, 4)
	var _drag_accel:Vector3 = velocity * _drag_coef
	var _local_gravity_dir:Vector3 = get_local_gravity(position)
...
func get_depth(position:Vector3)->float:
	return GROUND_MARGIN_METRES - _get_unsigned_local_height(position)

func go_home_accel(position:Vector3)->Vector3:
	var _proj_on_world_plane = position;
	...
	var _target:Vector3 = _grnd_orig - get_local_gravity(position) * GO_HOME_POINT_H;
	
	return (_target-position) * GO_HOME_COEF

func is_in_outer_space(position:Vector3)->bool:
	var _proj_on_world_plane = position
	_proj_on_world_plane.z = 0.0
	_proj_on_world_plane = _proj_on_world_plane.normalized()
	...
	return ((position - _grnd_orig).length()) ** 2 > SQ_HALF_TER_WIDTH

func _get_unsigned_local_height(position:Vector3) -> float:
	var _proj_on_world_plane = position
	_proj_on_world_plane.z = 0.0
	_proj_on_world_plane = _proj_on_world_plane.normalized()
	...
	var _grnd_orig:Vector3 = _proj_on_world_plane * _terrain_radius
	var _grnd_to_pos:Vector3 = position - _grnd_orig
	
	return abs(_grnd_to_pos.dot(_grnd_normal))

func get_local_gravity(position:Vector3)->Vector3:
	var _proj_on_world_plane:Vector3 = position
	_proj_on_world_plane.z = 0.0
	_proj_on_world_plane = _proj_on_world_plane.normalized()
